...
class Debugger:
    ...
    def end_function(self, func):
        try:
            if self.call_stack[-1] != func:
                err_msg = "Not matched the call stack. %s != %s" % (
                    self.call_stack[-1],
                    func
                )
    ...
    def check_break_points(self):
        ...
        # all break points should be checked, because the 'expired' need
        # evaluated.
...
class Listeners(Listener):
    ...
    def start_keyword(self, keyword):
        for listener in self.listeners:
            listener.start_keyword(keyword)

    def end_keyword(self, keyword):
        for listener in self.listeners:
            listener.end_keyword(keyword)
