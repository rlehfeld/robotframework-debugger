"""
this module is support to run RDB web interface in standalone mode.
the RDB is running with robot, it's not available after the robot is stopped.
The user may be confused, "what's happened RDB? Does crushed?"

The RDB will switch over to stand alone mode to notify user the robot is
stopped friendly.
"""
...
class HttpServletApp:
    def __init__(self, environ, start_response):
        ...
        if action.func_defaults:
            reqiured_args_count = (
                action.func_code.co_argcount - len(action.func_defaults)
            )
        else:
        ...
        try:
            args, kw_args = self.__parse_args(
                self.params,
                reqiured_args,
                options_args,
            )
